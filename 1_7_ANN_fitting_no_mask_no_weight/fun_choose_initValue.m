...
global SDS_choosed_cw SDS_sim_correspond_cw SDS_choosed_tr SDS_sim_correspond_tr gate_sim_correspond gate_choosed mask
%% param
...
error_arr=error_arr(gate_choosed,SDS_choosed_tr,:);
total_error_arr(:,2)=squeeze(sqrt(mean(mean(error_arr.^2,1),2)));

if sum(isnan(total_error_arr))==0
    total_error=sqrt(mean(total_error_arr.^2,2));
else
    total_error=sqrt(mean(total_error_arr(~isnan(total_error_arr)).^2,2));
end
