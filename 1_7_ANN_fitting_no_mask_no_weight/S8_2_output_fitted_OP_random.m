...
generate random OP change to match the OP fitting error.
...
for i=1:length(target_name_arr)
    ...
    sbj_OP_CV_arr=load(fullfile(input_dir,[target_name_arr{i} '_OP_CV.txt']));
    sbj_OP_CV_arr=[sbj_OP_CV_arr(1:4) 0 0 sbj_OP_CV_arr(5:6) 0 0];
    for j=1:length(choosed_rank)
        % make the random error
        random_OP_change_arr=[];
        for L=1:10
            random_OP_change_arr(:,L)=normrnd(1,sbj_OP_CV_arr(L)/100,num_random_OP,1);
        end
