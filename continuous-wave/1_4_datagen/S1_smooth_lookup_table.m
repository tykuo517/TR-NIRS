                ...
                for i=1:size(lkt_points_ref,1)
                    for j=1:size(lkt_points_ref,2)
                        for k=1:size(lkt_points_ref,3)
                            ...
                            coefficients=polyfitn(interp_layer_mus{4}, data_1d, degree);
                            fitted_values=polyvaln(coefficients, interp_layer_mus{4});
                            fitted_values=power(10,-fitted_values);
                            
                            smoothed_data=interpn(interp_layer_mus{4},fitted_values,lkt_layer_mus{4},'spline');
                            
                            lkt_ref_value_arr_as((step-1)*size(lkt_points_ref,4)+1:step*size(lkt_points_ref,4),mua_i,s)=smoothed_data;
                            step=step+1;
                        end
                    end
                end
...
%                 lkt_ref_value_arr_as(:,mua_i,s)=lkt_value_as;
    ...
    dtof_arrange_hs=dtof_arrange_gm(length(lkt_layer_mus{4})+1:2*length(lkt_layer_mus{4}),:);
    dtof_arrange_ms=dtof_arrange_gm(1:length(lkt_layer_mus{4}),:);
...
%                 legend('1E10','polynomial','2E11','Location','northwest');
            ...
            if hs_or_ls_or_mid==0
               print(fullfile(output_dir,['hs_SDS_' num2str(SDS_array(s)) 'cm_smooth_result.png']),'-dpng','-r200');
            elseif hs_or_ls_or_mid==1
               print(fullfile(output_dir,['ls_SDS_' num2str(SDS_array(s)) 'cm_smooth_result.png']),'-dpng','-r200');
            elseif hs_or_ls_or_mid==2
               print(fullfile(output_dir,['ms_SDS_' num2str(SDS_array(s)) 'cm_smooth_result.png']),'-dpng','-r200');
            end
        end      
    end
